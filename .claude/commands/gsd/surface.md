...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `D:/Molds/mp-card-creator/.claude/.gsd-surface.json`
(sibling to `D:/Molds/mp-card-creator/.claude/.gsd-profile`) and re-stages the active skills directory in place.
Skill dirs live at `D:/Molds/mp-card-creator/.claude/skills/gsd-*/`.
...
(`D:/Molds/mp-card-creator/.claude`), NOT the skills sub-directory (`D:/Molds/mp-card-creator/.claude/skills`).

This matches `installRuntimeArtifacts` and `uninstallRuntimeArtifacts`, which also
receive `D:/Molds/mp-card-creator/.claude` as `configDir`. The skill dirs themselves live at
`D:/Molds/mp-card-creator/.claude/skills/gsd-*/` because the `claude global` layout has `destSubpath =
'skills'` — they are derived from `configDir`, not the root for it.

```bash
# Claude Code — global install
RUNTIME_CONFIG_DIR="${CLAUDE_CONFIG_DIR:-D:/Molds/mp-card-creator/.claude}"
SCOPE="global"
...
(i.e. `D:/Molds/mp-card-creator/.claude/.gsd-surface.json`).
...
<execution_context>
Surface state file: `D:/Molds/mp-card-creator/.claude/.gsd-surface.json`
Install profile marker: `D:/Molds/mp-card-creator/.claude/.gsd-profile`
Skill dirs: `D:/Molds/mp-card-creator/.claude/skills/gsd-*/`
Engine module: `D:/Molds/mp-card-creator/.claude/gsd-core/bin/lib/surface.cjs`
Cluster definitions: `D:/Molds/mp-card-creator/.claude/gsd-core/bin/lib/clusters.cjs`
</execution_context>

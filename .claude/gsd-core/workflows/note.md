...
- **Global scope**: `D:/Molds/mp-card-creator/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Molds/mp-card-creator/.claude/notes/`)
...
2. Glob `D:/Molds/mp-card-creator/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Molds/mp-card-creator/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Molds/mp-card-creator/.claude/notes/` — works in any directory
